...
**~54 of 69 issues resolved** across **27 verified commits** on `fix/audit-remediation`. All components green: `make checkall` from root exits 0 (backend **377** tests, frontend **149**, hooks **18**, opencode-plugin **29** — up from a 41/0/13/0 baseline; **+469 new tests**). Verified at runtime too: `make dev-tmux` + `make simulate` ran clean to `session_end`, every event type through the ARC-014 union, `/api/v1/status` no longer discloses the key (SEC-001), no ImportErrors. **All security, all documentation, the entire backend structural chain, and the bulk of code quality are done.** **~15 remain**: the frontend god-object refactors (the audit's *long-term/backlog* — deferred as too risky for one-shot automation) plus a short tail of medium/low items.
...
**Low (4):** ARC-019 (`gen_types` introspection) · ARC-029 (`install.sh` read-modify-write) · QA-013 (magic numbers) · QA-016 (God-component split).
