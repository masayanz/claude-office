...
$dist = Join-Path $root "dist\manager"
...
Write-Host "PyInstallerでClaudeOfficeManager.exeをビルドします。"
$command = "& $Python -m PyInstaller --noconfirm --clean --onefile --windowed --name ClaudeOfficeManager --icon `"$icon`" --distpath `"$dist`" --workpath `"$work`" --specpath `"$work`" `"$entry`""
Invoke-Expression $command
if ($LASTEXITCODE -ne 0) {
    throw "PyInstaller failed with exit code $LASTEXITCODE"
}

$exe = Join-Path $dist "ClaudeOfficeManager.exe"
