...
function New-ClaudeOfficeHandler([string]$launcherPath, [string]$shellName) {
    $launcherLiteral = "'" + $launcherPath.Replace("'", "''") + "'"
    $launcherCommand = "& $launcherLiteral"
    $encodedCommand = [Convert]::ToBase64String(
        [System.Text.Encoding]::Unicode.GetBytes($launcherCommand)
    )
    return [pscustomobject]@{
        type = "command"
        command = "python3 ~/.codex/claude-office-hook.py"
        commandWindows = "$shellName -NoProfile -ExecutionPolicy Bypass -EncodedCommand $encodedCommand"
        # Portable launchers may need more than two seconds to initialize on
        # Windows (especially on the first run after extraction).
        timeout = 10
    }
}
...
$shellName = [System.IO.Path]::GetFileName($shellCommand.Source)
...
if (Test-Path -LiteralPath $hooksPath) {
    ...
    Get-ChildItem -LiteralPath $backupDir -Filter "hooks-*.json" -File |
        ...
        Remove-Item -Force
    try {
        $config = ConvertFrom-Json -InputObject (Read-Utf8Text $hooksPath)
    } catch {
...
$handler = New-ClaudeOfficeHandler $launcherPath $shellName
...
try {
    # Read the raw redirected stream explicitly. [Console]::In may already have
    # been initialised with the OEM code page by Windows PowerShell 5.1.
    $utf8 = New-Object System.Text.UTF8Encoding($false, $true)
    $stdinReader = New-Object System.IO.StreamReader([Console]::OpenStandardInput(), $utf8, $false)
    $payload = $stdinReader.ReadToEnd()
    # Windows PowerShell 5.1 otherwise encodes native-pipeline input as ASCII.
    $OutputEncoding = New-Object System.Text.UTF8Encoding($false)
    $configPath = Join-Path $PSScriptRoot "claude-office-config.json"
    $config = ConvertFrom-Json -InputObject ([System.IO.File]::ReadAllText($configPath, $utf8))
    ...
    if ($adapterPath.ToLowerInvariant().EndsWith(".exe")) {
        ...
        # .NET Framework (Windows PowerShell 5.1) creates Process.StandardInput
        # with Console.InputEncoding and has no StandardInputEncoding property.
        [Console]::InputEncoding = New-Object System.Text.UTF8Encoding($false)
        $adapterProcess = New-Object System.Diagnostics.Process
        $adapterProcess.StartInfo = $startInfo
        $null = $adapterProcess.Start()
        $adapterProcess.StandardInput.Write($payload)
        $adapterProcess.StandardInput.Close()
        $adapterProcess.WaitForExit()
    } else {
        $pyLauncher = Get-Command py.exe -ErrorAction SilentlyContinue
        if ($pyLauncher) {
            $payload | & $pyLauncher.Source -3.13 $adapterPath
        } else {
            ...
            if ($pythonLauncher) { $payload | & $pythonLauncher $adapterPath }
        }
    }
} catch {
...
try:
    ...
    subprocess.run([sys.executable, str(hook)], check=False, stdin=sys.stdin)
except Exception:
    pass
'@
