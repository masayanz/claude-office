const en = {
  // App
  "app.title": "AI Office Viewer",
  "app.subtitle": "Visualize AI agent activity in real time",
  "app.initializingSystems": "Initializing Systems...",

  // Header Controls
  "header.simulate": "SIMULATE",
  "header.reset": "RESET",
  "header.clearDb": "CLEAR DB",
  "header.debugOn": "DEBUG ON",
  "header.debugOff": "DEBUG OFF",
  "header.settings": "SETTINGS",
  "header.help": "HELP",
  "header.status": "Status",
  "header.connected": "CONNECTED",
  "header.disconnected": "DISCONNECTED",
  "header.aiOn": "ON",
  "header.aiOff": "OFF",
  "header.codex": "CODEX",
  "header.codexLive": "LIVE",
  "header.codexWaiting": "WAITING",
  "header.codexRestored": "RESTORED",
  "header.codexHybrid": "HYBRID",
  "header.codexJsonl": "JSONL",
  "header.codexLastEvent": "Last event: {time}",
  "header.codexNoEvents": "Last event: none",
  "header.codexLiveEventCount": "Live events: {count}",
  "header.agents": "agents",
  "header.tour": "TOUR",
  "header.commandCenter": "COMMAND",
  "header.more": "MORE",
  "header.moreMenu": "More actions",
  "header.restoreCodexSessions": "RESTORE CODEX SESSIONS",
  "help.tour": "Take the Tour",

  // Command Center
  "commandCenter.title": "Command Center",
  "commandCenter.zone.needsYou": "Needs you",
  "commandCenter.zone.working": "Working",
  "commandCenter.zone.done": "Done",
  "commandCenter.zone.ended": "Ended",
  "commandCenter.moreCount": "+{count} more",
  "commandCenter.popup.todos": "Todos",
  "commandCenter.popup.employees": "Employees",
  "commandCenter.popup.terminal": "Terminal",
  "commandCenter.popup.drillIn": "Open",
  "commandCenter.initializing": "Initializing Command Center...",
  "commandCenter.board.allSessions": "ALL SESSIONS",
  "commandCenter.board.terminals": "Terminals",
  "commandCenter.board.employees": "Employees",
  "commandCenter.board.todos": "Todos",

  // Modals
  "modal.confirmDbWipe": "Confirm Database Wipe",
  "modal.cancel": "Cancel",
  "modal.wipeAllData": "Wipe All Data",
  "modal.wipeWarning":
    "Are you sure you want to permanently delete all session history and events? This action cannot be undone and will reset the current visualizer state.",
  "modal.keyboardShortcuts": "Keyboard Shortcuts",
  "modal.close": "Close",
  "modal.toggleDebug": "Toggle debug mode",
  "modal.showAgentPaths": "Show agent paths",
  "modal.showQueueSlots": "Show queue slots",
  "modal.showPhaseLabels": "Show phase labels",
  "modal.deleteSession": "Delete Session",
  "modal.delete": "Delete",
  "modal.deleteSessionConfirm": "Are you sure you want to delete session",
  "modal.deleteSessionWarning": "This will permanently remove",
  "modal.events": "events",
  "modal.cannotBeUndone": "This action cannot be undone.",

  // Settings
  "settings.title": "Settings",
  "settings.clockType": "Clock Type",
  "settings.analog": "Analog",
  "settings.digital": "Digital",
  "settings.timeFormat": "Time Format",
  "settings.12hour": "12-hour",
  "settings.24hour": "24-hour",
  "settings.clockTimezone": "Clock timezone",
  "settings.clockTimezoneAutomatic": "Automatic (PC local)",
  "settings.clockTimezoneCustom": "Custom timezone",
  "settings.clockTimezonePlaceholder": "e.g. Asia/Tokyo",
  "settings.mainAgentName": "Main Agent name",
  "settings.mainAgentNameDesc": "Use a custom name for the main agent",
  "settings.mainAgentNamePlaceholder": "e.g. My Main Agent",
  "settings.automatic": "Automatic",
  "settings.custom": "Custom",
  "settings.sessionBehavior": "Session Behavior",
  "settings.autoFollow": "Auto-follow new sessions",
  "settings.autoFollowDesc":
    "Automatically switch to new sessions in the current project",
  "settings.restoreCodexSessions": "Restore active Codex sessions on startup",
  "settings.restoreCodexSessionsDesc":
    "Detect recent Codex sessions and continue displaying their current state",
  "settings.restoreWindowMinutes": "Restore sessions active within (minutes)",
  "settings.restoreWindowMinutesDesc":
    "Sessions older than this window are not restored (1–1440 minutes).",
  "settings.restoreSaved": "Restore settings saved",
  "settings.restoreSaveFailed": "Could not save restore settings",
  "settings.clockTip":
    "Tip: Click the clock in the office to quickly cycle between modes.",
  "settings.language": "Language",
  "settings.characterActivity": "Character activity",
  "settings.characterActivityDesc": "Control secondary movement and ambient actions",
  "settings.characterActivityLevel.quiet": "Quiet",
  "settings.characterActivityLevel.standard": "Standard",
  "settings.characterActivityLevel.lively": "Lively",
  "settings.characterThinkingWalk": "Walk while thinking",
  "settings.characterIdeaEffects": "Show idea effects",
  "settings.characterBreakActions": "Allow short breaks",
  "settings.characterInteractions": "Show occasional character interactions",
  "settings.tabs.general": "General",
  "settings.tabs.building": "Building",
  "settings.tabs.office": "AI Office Viewer",
  "settings.tabs.board": "Whiteboard",
  "settings.office.companyName": "Company name",
  "settings.office.ownerName": "Owner name",
  "settings.office.owner": "Owner",
  "settings.office.ownerTitle": "Title (optional)",
  "settings.office.ownerMessage": "Message (optional)",
  "settings.office.resetOwnerImage": "Restore default image",
  "settings.office.ownerImage": "Owner image",
  "settings.office.backendPort": "Backend port",
  "settings.office.frontendPort": "Frontend port",
  "settings.office.browserMode": "Browser mode",
  "settings.office.browserNormal": "Normal browser",
  "settings.office.browserApp": "App window",
  "settings.office.save": "Save AI Office Viewer settings",
  "settings.office.saved": "Saved",
  "settings.office.saveFailed": "Could not save settings",
  "settings.office.imageSaved": "Owner image saved",
  "settings.office.imageFailed": "Could not save owner image",
  "settings.office.restartHint":
    "Port changes take effect after restarting AI Office Viewer.",
  "settings.building.name": "Building Name",
  "settings.building.namePlaceholder": "My Company",
  "settings.building.floors": "Floors",
  "settings.building.addFloor": "Add Floor",
  "settings.building.floorName": "Floor Name",
  "settings.building.floorNumber": "Floor",
  "settings.building.accentColor": "Accent",
  "settings.building.icon": "Icon",
  "settings.building.repos": "Project folders",
  "settings.building.reposPlaceholder": "claude-office, my-api, web-ui",
  "settings.building.save": "Save",
  "settings.building.saving": "Saving...",
  "settings.building.saved": "Saved!",
  "settings.building.deleteFloor": "Remove",
  "settings.building.noFloors": "No floors configured",
  "settings.building.enableHint":
    "Add floors and map project folder names to enable building navigation view",
  "settings.building.unsavedWarning":
    "You have unsaved building changes. Discard them?",
  "settings.building.saveFailed": "Save failed: {status} {statusText}",
  "settings.building.saveUnreachable":
    "Cannot reach backend — is the server running?",

  // Sessions
  "sessions.title": "Sessions",
  "sessions.loading": "Loading sessions...",
  "sessions.noSessions": "No sessions found",
  "sessions.unknownProject": "Unknown Project",
  "sessions.deleteSession": "Delete session",
  "sessions.events": "events",
  "sessions.events_one": "{count} event",
  "sessions.events_other": "{count} events",
  "sessions.expandSidebar": "Expand sidebar",
  "sessions.collapseSidebar": "Collapse sidebar",
  "sessions.dragToResize": "Drag to resize",

  // Right Sidebar
  "sidebar.events": "Events",
  "sidebar.conversation": "Conversation",

  // Event Log
  "eventLog.title": "Event Log",
  "eventLog.events": "events",
  "eventLog.events_one": "{count} event",
  "eventLog.events_other": "{count} events",
  "eventLog.waiting": "Waiting for events...",
  "eventLog.eventType": "Event type",

  // Agent Status
  "agentStatus.title": "Agent State",
  "agentStatus.agents": "agents",
  "agentStatus.agents_one": "{count} agent",
  "agentStatus.agents_other": "{count} agents",
  "agentStatus.noAgents": "No agents",
  "agentStatus.agent": "Agent",
  "agentStatus.desk": "Desk",
  "agentStatus.noTaskSummary": "No task summary",
  "agentStatus.noRecentToolCall": "No recent tool call",
  "agentStatus.recentTool": "Recent tool: {tool}",
  "agentStatus.inQueue": "In {queueType} queue (position {position})",
  "agentStatus.status.error": "Error",
  "agentStatus.status.departing": "Departing",
  "agentStatus.status.walking": "Walking",
  "agentStatus.status.thinking": "Thinking",
  "agentStatus.status.preparing": "Preparing",
  "agentStatus.status.working": "Working",
  "agentStatus.status.reviewing": "Reviewing",
  "agentStatus.status.waiting": "Waiting",
  "agentStatus.status.completed": "Done",
  "agentStatus.status.idle": "Idle",
  "agentStatus.bubble.thinking": "Thinking...",
  "agentStatus.bubble.preparing": "Preparing...",
  "agentStatus.bubble.working": "Working",
  "agentStatus.bubble.reviewing": "Reviewing",
  "agentStatus.bubble.waiting": "Waiting",
  "agentStatus.bubble.completed": "Done",
  // Character messages and secondary visual-activity labels. These are
  // separate from backend event text so ambient animation does not create a
  // new event-log entry.
  "agent.message.thinking": "Thinking…",
  "agent.message.thinkingAlt": "Sorting through ideas…",
  "agent.message.working": "Working",
  "agent.message.reviewing": "Checking the result",
  "agent.message.waiting": "Waiting for the next step",
  "agent.message.completed": "Done!",
  "agent.message.departing": "Signing off",
  "agent.message.idea": "I have an idea!",
  "agent.message.break": "Taking a short break",
  "agent.message.stretch": "Stretch break",
  "agent.message.walking": "On the move",
  "agent.message.whiteboard": "Checking the whiteboard",
  "agent.message.window": "Looking outside",
  "agent.message.coffee": "Coffee break",
  "agent.message.error": "Something went wrong",
  "agent.message.interaction": "Quick hello!",
  "agent.message.arrivalTask": "Here's your task, {name}!",
  "agent.message.arrivalAccepted": "Got it!",
  "agent.message.departureHandoff": "Good work, {name}. I'll take that.",
  "agent.message.departureCompleted": "Task complete!",
  "agent.message.farewell": "See you!",
  "agent.activity.thinking": "Thinking at the desk",
  "agent.activity.pacing": "Moving while thinking",
  "agent.activity.whiteboard": "Checking the whiteboard",
  "agent.activity.window": "Looking outside",
  "agent.activity.coffee": "Taking a break",
  "agent.activity.stretch": "Stretching",
  "agent.activity.typing": "Typing",
  "agent.activity.monitor": "Checking the monitor",
  "agent.activity.document": "Reviewing a document",
  "agent.activity.completed": "Completed",
  "agent.activity.error": "Error",
  "agent.activity.walking": "Walking",
  "agent.activity.interaction": "Saying hello",
  "agent.activity.idle": "Idle",
  "agentStatus.phase.idle": "Idle",
  "agentStatus.phase.arriving": "Arriving",
  "agentStatus.phase.in_arrival_queue": "In arrival queue",
  "agentStatus.phase.walking_to_ready": "Walking to ready area",
  "agentStatus.phase.conversing": "Conversing",
  "agentStatus.phase.walking_to_boss": "Walking to main agent",
  "agentStatus.phase.at_boss": "At main agent",
  "agentStatus.phase.walking_to_desk": "Walking to desk",
  "agentStatus.phase.departing": "Departing",
  "agentStatus.phase.in_departure_queue": "In departure queue",
  "agentStatus.phase.walking_to_elevator": "Walking to elevator",
  "agentStatus.phase.in_elevator": "In elevator",

  // Main agent names
  "mainAgent.codex": "Codex Main",
  "mainAgent.claude": "Claude Main",
  "mainAgent.opencode": "OpenCode Main",
  "mainAgent.default": "AI Main",

  // Connection
  "connection.connected": "Connected",
  "connection.disconnected": "Disconnected",
  "connection.connecting": "Connecting",

  // Event types (wire values remain unchanged)
  "eventType.session_start": "Session started",
  "eventType.session_end": "Session ended",
  "eventType.pre_tool_use": "Tool started",
  "eventType.post_tool_use": "Tool finished",
  "eventType.user_prompt_submit": "Instruction received",
  "eventType.permission_request": "Permission request",
  "eventType.notification": "Notification",
  "eventType.subagent_start": "Subagent started",
  "eventType.subagent_info": "Subagent info",
  "eventType.subagent_stop": "Subagent stopped",
  "eventType.agent_update": "Agent update",
  "eventType.stop": "Turn completed",
  "eventType.cleanup": "Cleanup",
  "eventType.context_compaction": "Context compaction",
  "eventType.reporting": "Reporting",
  "eventType.walking_to_desk": "Walking to desk",
  "eventType.waiting": "Waiting",
  "eventType.leaving": "Leaving",
  "eventType.error": "Error",
  "eventType.background_task_notification": "Background task",
  "eventType.task_created": "Task created",
  "eventType.task_completed": "Task completed",
  "eventType.teammate_idle": "Teammate idle",

  // Git Status
  "git.title": "Git Status",
  "git.waitingForStatus": "Waiting for git status...",
  "git.noSession": "No session selected",
  "git.noRepo": "No git repository detected",
  "git.changedFiles": "Changed Files",
  "git.staged": "staged",
  "git.recentCommits": "Recent Commits",
  "git.noCommits": "No commits found",
  "git.modified": "modified",
  "git.added": "added",
  "git.deleted": "deleted",
  "git.renamed": "renamed",
  "git.copied": "copied",
  "git.untracked": "untracked",
  "git.ignored": "ignored",

  // Conversation
  "conversation.title": "Conversation",
  "conversation.msgs": "msgs",
  "conversation.msgs_one": "{count} msg",
  "conversation.msgs_other": "{count} msgs",
  "conversation.thinking": "Thinking",
  "conversation.showMore": "Show more",
  "conversation.collapse": "Collapse",
  "conversation.claude": "Claude",
  "conversation.showFullResponse": "Show full response",
  "conversation.hideToolCalls": "Hide tool calls",
  "conversation.showToolCalls": "Show tool calls",
  "conversation.expandConversation": "Expand conversation",
  "conversation.close": "Close",
  "conversation.noConversation":
    "No conversation yet. Start a Claude Code session.",

  // Event Detail Modal
  "eventDetail.summary": "Summary",
  "eventDetail.tool": "Tool",
  "eventDetail.agentName": "Agent Name",
  "eventDetail.taskDescription": "Task Description",
  "eventDetail.userPrompt": "User Prompt",
  "eventDetail.thinking": "Thinking",
  "eventDetail.message": "Message",
  "eventDetail.resultSummary": "Result Summary",
  "eventDetail.errorType": "Error Type",
  "eventDetail.toolInput": "Tool Input",
  "eventDetail.noDetail": "No additional detail available for this event.",

  // Loading Screen
  "loading.office": "Loading office...",

  // Zoom Controls
  "zoom.in": "Zoom in",
  "zoom.out": "Zoom out",
  "zoom.reset": "Reset zoom",

  // Mobile
  "mobile.menu": "Menu",
  "mobile.agentActivity": "Agent Activity",
  "mobile.boss": "BOSS",
  "mobile.noActiveAgents": "No active agents",

  // Status Messages
  "status.switchedToSession": "Switched to session {sessionId}...",
  "status.deletingSession": "Deleting session {sessionId}...",
  "status.sessionDeleted": "Session deleted.",
  "status.failedDeleteSession": "Failed to delete session.",
  "status.errorConnecting": "Error connecting to backend.",
  "status.sessionRenamed": "Session renamed.",
  "status.failedRenameSession": "Failed to rename session.",
  "status.clearingDatabase": "Clearing database...",
  "status.databaseCleared": "Database cleared.",
  "status.failedClearDatabase": "Failed to clear database.",
  "status.triggeringSimulation": "Triggering simulation...",
  "status.simulationStarted": "Simulation started!",
  "status.failedSimulation": "Failed to trigger simulation.",
  "status.storeReset": "Store reset.",
  "status.sessionDeletedSwitched": "Session deleted. Switched to {sessionName}",
  "status.sessionDeletedNoOthers":
    "Session deleted. No other sessions available.",
  "status.connectedTo": "Connected to {sessionName}",
  "status.autoFollowed": "Auto-followed new session: {sessionName}",
  "status.restoringCodexSessions": "Checking active Codex sessions...",
  "status.codexSessionsRestored":
    "Restored {sessionCount} Codex session(s) and {agentCount} agent(s).",
  "status.failedRestoreCodexSessions":
    "Could not restore Codex sessions. Codex itself is not affected.",

  // Navigation
  "navigation.building": "Building",

  // Tour
  "tour.steps.welcome.title": "Welcome",
  "tour.steps.welcome.description":
    "This is your command center. Click a floor to explore.",
  "tour.steps.singleWelcome.title": "Welcome to AI Office Viewer",
  "tour.steps.singleWelcome.description":
    "This pixel art office shows your Claude Code sessions in real time. Let's take a quick look around.",
  "tour.steps.simulate.title": "Start Simulation",
  "tour.steps.simulate.description":
    "Click Simulate to bring the office to life.",
  "tour.steps.agentsArrive.title": "Agents Arrive",
  "tour.steps.agentsArrive.description":
    "Agents arrive through the elevator, walk to their desks, and start working.",
  "tour.steps.inspectAgent.title": "Inspect an Agent",
  "tour.steps.inspectAgent.description":
    "Click on any character to inspect them.",
  "tour.steps.focusPopup.title": "Focus Popup",
  "tour.steps.focusPopup.description":
    "From here you can copy a message to clipboard and jump to your terminal. The office updates in real time as Claude works.",
  "tour.steps.zoomOut.title": "Zoom Out",
  "tour.steps.zoomOut.description":
    "Try zooming back out to see the big picture.",
  "tour.steps.settings.title": "Configure Floors",
  "tour.steps.settings.description":
    "Open Settings to add floors and map project folders for a multi-floor building view.",
  "tour.skip": "Skip tour",
  "tour.next": "Next",

  // Attention System
  "attention.toast.permissionRequest": "{agentName} needs permission",
  "attention.toast.error": "{agentName} encountered an error",
  "attention.toast.taskCompleted": "{agentName} completed a task",
  "attention.toast.agentArrived": "{agentName} joined the office",
  "attention.toast.stop": "{agentName} stopped",
  "attention.toast.backgroundTask": "{agentName}: {taskDescription}",
  "attention.commandBar.placeholder": "Type a command...",
  "attention.commandBar.focusAgent": "Focus Agent: {name}",
  "attention.commandBar.focusBoss": "Focus Boss Terminal",
  "attention.commandBar.showAttention": "Show Attention Queue",
  "attention.commandBar.dismissAll": "Dismiss All Toasts",
  "attention.commandBar.toggleDebug": "Toggle Debug View",
  "attention.commandBar.togglePaths": "Toggle Path Display",
  "attention.commandBar.toggleQueueSlots": "Toggle Queue Slots",
  "attention.commandBar.togglePhaseLabels": "Toggle Phase Labels",
  "attention.commandBar.toggleObstacles": "Toggle Obstacles",
  "attention.commandBar.noResults": "No matching commands",
  "attention.popup.focusTerminal": "Focus Terminal",
  "attention.popup.close": "Close",
  "attention.popup.state": "State",
  "attention.popup.task": "Task",
  "attention.popup.type": "Type",
  "attention.popup.desk": "Desk",
  "attention.popup.boss": "Boss",
  "attention.popup.agent": "Agent",
  // Whiteboard canvas
  "whiteboard.noTasks": "No tasks yet",
  "whiteboard.noKanbanTasks": "No kanban tasks",
  "whiteboard.noRemoteWorkers": "No remote workers",
  "whiteboard.noNews": "No news yet - stay tuned!",
  "whiteboard.breaking": "📰 BREAKING",
  "whiteboard.noFileEdits": "No file edits yet",
  "whiteboard.successfulToolUses": "SUCCESSFUL TOOL USES",
  "whiteboard.sinceLastIncident": "since last incident",
  "whiteboard.boss": "👔 BOSS",
  "whiteboard.supervising": "Supervising",
  "whiteboard.noEmployees": "No employees yet",
  "whiteboard.noAgentActivity": "No agent activity yet",
  "whiteboard.noToolsUsed": "No tools used",
  "whiteboard.todo": "TODO",
  "whiteboard.inProgress": "IN PROGRESS",
  "whiteboard.done": "DONE",
  "whiteboard.more": "+{count} more",
  "whiteboard.coffeeTracker": "☕ COFFEE TRACKER",
  "whiteboard.cupsConsumed": "cups consumed",
  "whiteboard.weather.stormy": "STORMY",
  "whiteboard.weather.rainy": "RAINY",
  "whiteboard.weather.cloudy": "CLOUDY",
  "whiteboard.weather.sunny": "SUNNY",
  "whiteboard.success": "Success",
  "whiteboard.errors": "Errors",
  "whiteboard.activity": "Activity",
  "whiteboard.totalOps": "Total ops",
  "settings.commandBar": "Command Bar (⌘K)",
  "settings.clickToFocus": "Click to Focus",
  "settings.toastFilters": "Toast Notifications",
  "settings.toastAutoDismiss": "Auto-dismiss Timing",
  "settings.filterPermission": "Permission requests",
  "settings.filterError": "Errors and stops",
  "settings.filterTaskComplete": "Task completions",
  "settings.filterArrival": "Agent arrivals",
  "replay.live": "Live",
  "replay.replay": "Replay",
  "replay.title": "Work history replay",
  "replay.selectSession": "Select a session",
  "replay.noSessions": "No replay sessions found",
  "replay.historyDisabled": "Work history recording is disabled",
  "replay.historyEmpty": "No work history is available to replay yet",
  "replay.filteredEmpty": "No history matches the current filters",
  "replay.apiError": "Work history could not be loaded",
  "replay.eventEmpty": "This session has no replayable events",
  "replay.project": "Project",
  "replay.model": "Model",
  "replay.fromDate": "From date",
  "replay.toDate": "To date",
  "replay.source": "Source",
  "replay.allSources": "All sources",
  "replay.newest": "Newest first",
  "replay.oldest": "Oldest first",
  "replay.events": "events",
  "replay.agents": "agents",
  "replay.inProgress": "In progress",
  "replay.recordedAt": "Recorded",
  "replay.play": "Play",
  "replay.pause": "Pause",
  "replay.beginning": "Beginning",
  "replay.back10": "Back 10 seconds",
  "replay.forward10": "Forward 10 seconds",
  "replay.nextEvent": "Next event",
  "replay.speed": "Playback speed",
  "replay.completed": "Replay complete",
  "replay.returnLive": "Return to Live",
  "replay.repeat": "Replay again",
  "replay.compressIdle": "Long idle gaps shortened",
  "replay.liveClock": "Current time",
  "replay.loading": "Loading replay…",
  "settings.replay.title": "Work history & replay",
  "settings.replay.enabled": "Save work history",
  "settings.replay.retention": "Retention period",
  "settings.replay.compressIdle": "Shorten long idle gaps",
  "settings.replay.defaultSpeed": "Default speed",
  "settings.replay.clock": "Replay clock",
  "settings.replay.recorded": "Recorded time",
  "settings.replay.current": "Current time",
} as const;

export type TranslationKey = keyof typeof en;
export default en;
